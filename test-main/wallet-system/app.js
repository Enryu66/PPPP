...
app.post('/send-trc20-transaction', async (req, res) => {
  try {
    const client = new TronWeb({
      fullHost: "https://nile.trongrid.io/",
      privateKey: 'bf4fdf31c897a993cd4b27d42073deb1d574ed977f5ca5807fe64e248ec25d4d',
    });
    ...
    const trxBalance = await client.trx.getBalance(fromAddress);

    // Use TronWeb to load the TRC20 contract instance
    const contract = await client.contract().at(contractAddress);
    const decimals = await contract.decimals().call(); // Get the number of decimals from the TRC20 contract

    // Calculate the token amount with the correct precision using TronWeb's utility
    const amountWithDecimals = client.toHex(amount * 10 ** decimals);

    // Call the transfer function in the TRC20 contract
    const transaction = await contract.transfer(toAddress, amountWithDecimals).send({
      shouldPollResponse: true,
      feeLimit: 30000000, // Transaction fee limit
    });
    ...
    res.json({
      success: true,
      transaction: transaction.txid // เพิ่มรหัสทำธุรกรรมใน response JSON
    });
  } catch (error) {
    console.error('เกิดข้อผิดพลาด:', error);
    res.status(500).json({
